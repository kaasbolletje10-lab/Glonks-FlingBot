...
local function isOwner(username)
	for _, ownerName in ipairs(OWNERS) do
		if username == ownerName then return true end
	end
	return false
end
...
local function handleCommand(player, msg)
	...
	elseif cmd == ".cmd" then
		sendChatMessage(".summon .stop .orbit [n] .tp .tpwall1 .tpwall2  .wl [user] .unwl [user] .opp [user] .fling [user/all] .stopfling .spin [n] .1 .2 .3 .4 .status .rj .re .script .ver")
...
for _, player in ipairs(Players:GetPlayers()) do
	if player ~= host and not isOwner(player.Name) then
		player.Chatted:Connect(function(msg)
			handleCommand(player, msg)
		end)
	end
